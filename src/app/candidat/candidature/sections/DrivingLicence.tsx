...
export const DrivingLicenceSection: React.FC<DrivingLicenceSectionProps> = ({
  ...
  const licenceCategories = [
    ...
    {
      value: "B1",
      label: "B1 - PKW bis 3,5t (Voiture particulière jusqu’à 3,5 t)",
    },
    { value: "C", label: "C - LKW (Poids lourd)" },
    {
      value: "C1",
      label: "C1 - LKW bis 7,5t (Poids lourd jusqu’à 7,5 t)",
    },
    {
      value: "CE",
      label: "CE - LKW mit Anhänger (Poids lourd avec remorque)",
    },
    { value: "D", label: "D - Bus (Autobus)" },
    {
      value: "D1",
      label: "D1 - Bus bis 16 Pers. (Autobus jusqu’à 16 personnes)",
    },
    {
      value: "BE",
      label: "BE - PKW mit Anhänger (Voiture particulière avec remorque)",
    },
  ];
  ...
  return (
    <div className="space-y-6">
      <p className="font-body-md text-sm text-on-surface-variant/70">
        Angaben zum Führerschein (für viele Berufe in Deutschland wichtig).{" "}
        (Informations concernant le permis de conduire, important pour de
        nombreux métiers en Allemagne.)
      </p>
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-4">
            <label className="font-label-md text-label-md text-brand-imperial">
              Besitzen Sie einen Führerschein? (Possédez-vous un permis de
              conduire ?)
            </label>
          ...
          {data.hasLicence && (
            <>
              <div className="ml-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="block font-label-md text-xs text-brand-imperial">
                    Führerschein seit (Permis obtenu depuis){" "}
              ...
              <div className="ml-6 space-y-3">
                <label className="block font-label-md text-xs text-brand-imperial">
                  Führerscheinklassen (Catégories de permis){" "}
                  <span className="text-error">*</span>
                </label>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                  {licenceCategories.map((cat) => (
                    <label
                      key={cat.value}
                      className="flex items-center gap-2 cursor-pointer p-2 hover:bg-brand-ice/20 rounded-lg transition-colors"
                    >
                      <input
                        type="checkbox"
                        name="category"
                ...
                {/* Free text for "Welche" */}
                <div className="space-y-2 pt-2">
                  <label className="block font-label-md text-xs text-brand-imperial">
                    Welche (Lesquels) — falls nicht aufgeführt (si non listé)
                  </label>

                  <input
                    type="text"
                    name="categoriesText"
                    value={data.categoriesText || ""}
                    onChange={handleChange}
                    className="w-full bg-white border border-outline-variant rounded-lg px-4 py-2 font-body-md text-body-md focus:outline-none focus:ring-2 focus:ring-secondary focus:border-transparent transition-shadow"
                    placeholder="Weitere Führerscheinklassen (Autres catégories de permis)"
                  />
                </div>
              </div>
            </>
      ...
      <div className="mt-4 p-4 bg-brand-ice/10 rounded-lg border border-brand-imperial/10">
        <p className="text-sm text-on-surface-variant/70 flex items-center gap-2">
          ...
          Ein gültiger Führerschein ist für viele Berufe in Deutschland eine
          wichtige Voraussetzung. (Un permis de conduire valide est une
          condition importante pour de nombreux métiers en Allemagne.)
        </p>
      </div>
    </div>
  );
